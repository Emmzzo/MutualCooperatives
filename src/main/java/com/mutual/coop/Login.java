...
import mutual.dao.impl.LoginHistoricImpl;
...
//				his.setCreatedBy(user.getFname() + " " + user.getLname());
			...
			if (user != null && user.getUserCategory().getUserCatid() == 1) {// ADMIN
				...
				// return "";
			} else if (user != null && user.getUserCategory().getUserCatid() == 2) {// MUTUAL REP

				LOGGER.info(CLASSNAME + ":::MUTUAL REP ");
				return "RepPage1.xhtml?faces-redirect=true";
			}else if (user != null && user.getUserCategory().getUserCatid() == 3) {// MEMBER

				LOGGER.info(CLASSNAME + ":::MEMBER ");
				return "MemberPage1.xhtml?faces-redirect=true";
			}else if (user != null && user.getUserCategory().getUserCatid() == 4) {// GUEST 

				LOGGER.info(CLASSNAME + ":::GUEST ");
				return "GuestPage1.xhtml?faces-redirect=true";
			}else {
				LOGGER.info(CLASSNAME + ":::USER NOT FOUND ");
				return null;
			}
